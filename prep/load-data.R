...
#   left_join(cali_dois) %>% 
#   mutate(doi = if_else(str_detect(pmid, "10\\."), str_extract(pmid, "10\\..*"), doi))
# 
...
institutions_ncts_primary <- cali_unis$search_terms %>% 
  map(~search_NCTS_lead_sponsor(AACT_datasets, .)[[1]]) %>%
...
# CTgov_sample_Cali <- list()
# CTgov_sample_Cali[[1]] <- CTgov_sample_full %>%
...
write_csv(cali_prosp_reg, here("data", "processed", "cali_prospective_registration.csv"))
